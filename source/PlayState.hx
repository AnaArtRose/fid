	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'museum':
			{
						defaultCamZoom = 0.9;
						curStage = 'museum';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('museum/bg', 'weekh'));
						bg.antialiasing = true;
			...
			case 'museumEvil':
			{
						defaultCamZoom = 0.9;
						curStage = 'museumEvil';
						var bg:FlxSprite = new FlxSprite(-600, -350).loadGraphic(Paths.image('museum/bgEvil','weekh'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
			...
			case 'museumBonus':
			{
						defaultCamZoom = 0.9;
						curStage = 'museum';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('museum/bgBonus', 'weekh'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						bg.screenCenter();
						...
						/*background = new FlxSprite(-500, -250);
						background.frames = Paths.getSparrowAtlas('museum/background','weekh');
						background.animation.addByPrefix('paper', 'background', 24, false);
						background.animation.play('paper');
						background.antialiasing = true;
						background.scrollFactor.set(0.9, 0.9);
						background.setGraphicSize(Std.int(background.width * 0.85));
						background.updateHitbox();
						if(FlxG.save.data.distractions){
							add(background);

						var foreground = new FlxSprite(-500, -250);
						foreground.frames = Paths.getSparrowAtlas('museum/foreground','weekh');
						foreground.animation.addByPrefix('paper', 'foreground', 24, false);
						foreground.animation.play('paper');
						foreground.antialiasing = true;
						foreground.scrollFactor.set(0.9, 0.9);
						foreground.setGraphicSize(Std.int(background.width * 0.85));
						foreground.updateHitbox();
							add(foreground);

						var confetti = new FlxSprite(-500, -250);
						confetti.frames = Paths.getSparrowAtlas('museum/confetti','weekh');
						confetti.animation.addByPrefix('confect', 'confettgif', 24, false);
						confetti.animation.play('confect');
						confetti.scrollFactor.set(0.5, 0.5);
